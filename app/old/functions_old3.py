...
def query_document(vectorstore, fields_list, api_key, model_name):
    ...
    :param fields_list: List of fields that the user wants to extract
    :param api_key: The OpenAI API key to use when calling the OpenAI Embeddings API
    :param model_name: The OpenAI model to use (selected by user)
    ...
    query = f"Give me the {', '.join(fields_list)} from the context of the document."

    rag_chain = (
        {"context": retriever | format_docs, "question": RunnablePassthrough()}
        | prompt_template
        | llm.with_structured_output(DynamicModel, strict=True)
    )
