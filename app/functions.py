...
def query_document_per_file(vectorstore, fields_list, api_key, model_name, document_names):
    """
    ...
    retriever = vectorstore.as_retriever(
        ...
        search_kwargs={
            "k": 15  # Get more chunks for better context when dealing with multiple documents
        }
    )
    ...
    for doc_name in document_names:
        ...
        1. The exact information found in "{doc_name}"
        ...
        rag_chain = (
            ...
            | prompt_template
            | llm.with_structured_output(DynamicModel, strict=False)
        )
...
def query_document(vectorstore, fields_list, api_key, model_name):
    ...
    retriever = vectorstore.as_retriever(
        search_type="similarity",
        search_kwargs={
            "k": 12  # Get more chunks for better context when dealing with multiple documents
        }
    )
